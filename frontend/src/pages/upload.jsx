...
// Import your SVG files as URLs for Vite
import errorIconUrl from "../assets/icons/close-circle.svg"; 
// ++ New import for the success icon ++
import successIconUrl from "../assets/icons/tick-circle.svg"; 
...
const supportedFileSVG = (color = "black") => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="20"
    height="20"
...
function UploadIdentity() {
  ...
  // ++ New state variable for successful file selection ++
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      ...
      if (file.size > maxSize) {
        ...
        setFileError(false);
        setFileName(file.name);
        // ++ Set success state on valid file ++
    ...
    <div className="flex items-center justify-center p-0 m-0 min-h-screen w-full bg-white font-poppins">
      <div className="min-h-screen w-full bg-white flex flex-col">
        ...
        <div className="flex flex-col items-center flex-1 justify-center max-w-md mx-auto w-full">
          ...
          <div
            onClick={handleFileClick}
            className={`w-full max-w-[350px] min-h-[139px] rounded-[24px] border-4 border-dotted p-6 flex flex-col items-center justify-center text-center transition 
              fileError // If there's a file error, red 
                ? "border-red-500 cursor-not-allowed"
                : fileUploadedSuccessfully // If no error but file is successful, green
                ? "border-green-500 cursor-pointer"
                : "border-[#050150] hover:bg-purple-50 cursor-pointer" // Default purple
            }`}
          >
            <div className="bg-transparent p-4 mb-2">
              <button
          ...
          <div className="flex justify-between items-center w-full max-w-[350px]">
            <div className="flex items-center gap-1">
              <button
                ...
                className="flex w-[15px] h-[15px] justify-center items-center bg-transparent border-none p-0 m-0"
              >
                {supportedFileSVG(fileError ? "red" : "black")}
              </button>
              <p
                className={`text-[10px] font-normal leading-[20px] ${
                  fileError ? "text-red-500" : "text-black"
                }`}
              >
                {fileError
          ...
          <div className="flex gap-4 mt-6">
            <button
              ...
              className="rounded-[16px] border-[1.5px] border-[#050150] w-[200px] h-[40px] flex justify-center items-center gap-[10px] text-[10px] bg-transparent hover:bg-gray-100 transition cursor-pointer"
            >
              Cancel
            </button>
            <button
              disabled={fileError || !fileName}
              className={`w-[60px] h-[40px] flex justify-center items-center gap-[10px] rounded-[16px] text-[10px] transition cursor-pointer ${
                fileError || !fileName
                  ? "bg-gray-400 text-white cursor-not-allowed"
                  : "bg-[#050150] text-white hover:bg-indigo-800"
              }`}
            >
              Upload
            </button>
